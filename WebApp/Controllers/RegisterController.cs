    ...
    public class RegisterController : Controller
    {
        ...
        public async Task<IActionResult> Index(RegisterViewModel viewModel)
        {
            if (ModelState.IsValid) 
            {
                ...
                {
                    if (await _authenticationService.RegisterAsync(viewModel))                     return RedirectToAction("Index", "Login");
                } 

                ModelState.AddModelError("", "User with the same mail address already exists");
            }
